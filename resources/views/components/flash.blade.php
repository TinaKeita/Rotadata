    ...
    <div class="mb-5 space-y-2">
        @if (session('success'))
            <div x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 6000)"
                ...
                <button type="button" @click="show = false" aria-label="Close" class="shrink-0 text-emerald-500 hover:text-emerald-700 dark:hover:text-emerald-300">&times;</button>
            </div>
        @endif
        ...
        @if (session('warning'))
            <div x-data="{ show: true }" x-show="show" x-transition
                ...
                <button type="button" @click="show = false" aria-label="Close" class="shrink-0 text-amber-500 hover:text-amber-700 dark:hover:text-amber-300">&times;</button>
            </div>
        @endif
        ...
        @if (session('error'))
            <div x-data="{ show: true }" x-show="show" x-transition
                ...
                <button type="button" @click="show = false" aria-label="Close" class="shrink-0 text-red-500 hover:text-red-700 dark:hover:text-red-300">&times;</button>
            </div>
        @endif
    </div>
@endif
